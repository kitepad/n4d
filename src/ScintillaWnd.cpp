...
bool CScintillaWnd::Init(HINSTANCE hInst, HWND hParent, HWND hWndAttachTo)
{
    ...
    bool bUseD2D = GetInt64(DEFAULTS_SECTION, L"Direct2D") != 0;
    ...
    bool showFoldingMargin = GetInt64(DEFAULTS_SECTION, L"ShowFoldingMargin") != 0;
    m_scintilla.SetMarginMaskN(SC_MARGE_FOLDER, SC_MASK_FOLDERS);
    m_scintilla.SetMarginWidthN(SC_MARGE_FOLDER, showFoldingMargin ? CDPIAware::Instance().Scale(*this, 14) : 0);
    ...
    if (GetInt64(DEFAULTS_SECTION, L"CaretLineFrame") != 0)
...
void CScintillaWnd::UpdateLineNumberWidth() const
{
    ...
    m_scintilla.SetMarginSensitiveN(SC_MARGE_LINENUMBER, true);
}
...
void CScintillaWnd::SetupLexerForLang(const std::string& lang) const
{
    ...
    std::string sFontName = CUnicodeUtils::StdGetUTF8(GetString(DEFAULTS_SECTION, L"FontName", defaultFont.c_str()));
...
void CScintillaWnd::SetupDefaultStyles() const
{
    ...
    m_scintilla.SetIndentationGuides(static_cast<Scintilla::IndentView>(GetInt64(DEFAULTS_SECTION, L"Indent", SC_IV_LOOKBOTH)));
    ...
    else
    {
        ...
        {
            m_scintilla.SetElementColour(Scintilla::Element::CaretLineBack, theme.CurrentTheme().itemHover);
        }
        else
        {
            m_scintilla.SetElementColour(Scintilla::Element::CaretLineBack, toRgba(theme.CurrentTheme().itemHover, 75));
        }
    }
...
void CScintillaWnd::MarginClick(SCNotification* pNotification)
{
    ...
    if (pNotification->margin == SC_MARGE_FOLDER)
    {
        if (pNotification->modifiers == 2)
            m_scintilla.FoldAll(Scintilla::FoldAction::Toggle);
        else
            m_scintilla.ToggleFold(m_scintilla.LineFromPosition(pNotification->position));
    }
...
void CScintillaWnd::MatchBraces(BraceMatch what)
{
    ...
    bool isBraceHighlightText = GetInt64(DEFAULTS_SECTION, L"HighlightBrace");
    if ((braceAtCaret != -1) && (braceOpposite == -1))
    {
        m_scintilla.BraceBadLight(braceAtCaret);
        if (lastIndicatorLength && isBraceHighlightText)
        {
...
bool CScintillaWnd::AutoBraces(WPARAM wParam) const
{
    ...
    if ((wParam == '(') ||
        ...
        (wParam == '-'))
    {
        if (GetInt64(DEFAULTS_SECTION, L"AutoBrace") == 0)
            return false;
        char braceBuf[2]      = {0};
...
{
    m_scintilla.SetTabWidth(static_cast<uptr_t>(GetInt64(DEFAULTS_SECTION, L"TabWidth", 4)));
    if (ts == TabSpace::Default)
        m_scintilla.SetUseTabs(static_cast<uptr_t>(GetInt64(DEFAULTS_SECTION, L"UseTabs")));
    else
...
void CDocScroll::InitScintilla(CScintillaWnd* pScintilla)
{
    ...
    style |= GetInt64(DEFAULTS_SECTION, L"ScrollStyle") > 0 ? CSBS_MAPMODE : 0;
