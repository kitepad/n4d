...
CMainWindow::CMainWindow(HINSTANCE hInst, const WNDCLASSEX* wcx /* = nullptr*/)
    ...
    , m_tabTip(hResource)
{
    m_fileTreeVisible     = CIniSettings::Instance().GetInt64(L"View", L"FileTreeVisible", 0) != 0;
...
bool CMainWindow::RegisterAndCreateWindow()
{
    ...
    if (RegisterWindow(&wcx))
    {
        ...
        {
            int w = static_cast<int>(CIniSettings::Instance().GetInt64(L"View", L"FileTreeWidth", 200));
            SetFileTreeWidth(w);
...
void CMainWindow::HandleEditorContextMenu(LPARAM lParam) 
{
    ...
    if (menuIdx == 2)
    {
        CheckMenuItem(popupMenu, cmdEnableD2D, GetViewSettings(L"d2d") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdLinenumbers, GetViewSettings(L"linenumbers") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdAutoBraces, GetViewSettings(L"autobrace") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdAutocomplete, GetViewSettings(L"autocomplete") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdAutocMatchcase, GetViewSettings(L"autocompleteMatchCase") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdFoldingMargin, GetViewSettings(L"ShowFoldingMargin") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdHighlightBrace, GetViewSettings(L"bracehighlighttext") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdFrameCaretLine, GetViewSettings(L"caretlineframe") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdLineWrap, GetViewSettings(L"wrapmode", 0) ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdLineWrapIndent, GetViewSettings(L"wrapmodeindent") ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(popupMenu, cmdUseTabs, GetViewSettings(L"usetabs") ? MF_CHECKED : MF_UNCHECKED);
    }
    ...
    switch (uMsg)
    {
        ...
        case WM_NCHITTEST:
        {
            ...
            if (pt.y < m_allRects.tabs.bottom)
            {
                if (m_hoveredRect == TitlebarRect::Tabs)
                {
                    if (m_hoveredItemIdx != -1 && m_hoveredItemIdx != m_tipIdx && (GetAsyncKeyState(VK_CONTROL) & 0x8000))
                    {
                        const auto& doc = m_docManager.GetDocumentFromID(m_allTabs[m_hoveredItemIdx].id);
...
LRESULT CMainWindow::DoCommand(WPARAM wParam, LPARAM /*lParam*/)
{
    ...
    switch (id)
    {
        ...
        {
            auto val = CIniSettings::Instance().GetInt64(L"View", L"autocompleteMatchCase", 1) ? 0 : 1;
            CIniSettings::Instance().SetInt64(L"View", L"autocompleteMatchCase", val);
