...
bool CFileTree::Init(HWND hParent)
{
    ...
    CreateEx(0, WS_CHILD | WS_CLIPCHILDREN | WS_TABSTOP | TVS_HASBUTTONS |  TVS_LINESATROOT | TVS_DISABLEDRAGDROP | TVS_TRACKSELECT | TVS_FULLROWSELECT, hParent, nullptr, WC_TREEVIEW);
    if (!*this)
        return false;
...
void CFileTree::RefreshThread(HTREEITEM refreshRoot, const std::wstring& refreshPath, bool expanding)
{
    ...
    if (refreshRoot == TVI_ROOT)
    {
        ...
        // parent folder.
        m_bGotoParent = CIniSettings::Instance().GetInt64(L"View", L"FileTreeGotoParent", 1) != 0;
...
bool CFileTree::InsertItem(const FileTreeItem* item, HTREEITEM parent, HTREEITEM insertAfter, const std::wstring& activePath)
{
    ...
    //wchar_t        dots[] = L"..";
    //wchar_t   s    = (wchar_t &)(m_path.c_str());
    wchar_t dots[MAX_PATH] = {0};
    int     size        = static_cast<int>(m_path.size());
    if (size > MAX_PATH)
        size = MAX_PATH;

    wmemcpy_s(dots, size, m_path.c_str(), size);
    ...
    if (tvi.state == TVIS_BOLD && !item->isDot)
