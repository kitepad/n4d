...
void CFindReplaceDlg::DoInitDialog(HWND hwndDlg)
{
    ...
    bool searchSubFolders = GetInt64(SEARCHREPLACE_SECTION, L"SearchSubfolders") != 0;
    Button_SetCheck(GetDlgItem(*this, IDC_SEARCHSUBFOLDERS), searchSubFolders ? BST_CHECKED : BST_UNCHECKED);
    bool followTab = GetInt64(SEARCHREPLACE_SECTION, L"SearchFolderFollowtab") != 0;
...
void CFindReplaceDlg::SaveSettings()
{
    ...
    SetInt64(SEARCHREPLACE_SECTION, L"SearchSubfolders", searchSubFolders ? 1LL : 0LL);
    bool followTab = IsDlgButtonChecked(*this, IDC_SEARCHFOLDERFOLLOWTAB) == BST_CHECKED;
    SetInt64(SEARCHREPLACE_SECTION, L"SearchFolderFollowtab", followTab ? 1LL : 0LL);
    SaveSearchStrings();
    SaveReplaceStrings();
...
void CFindReplaceDlg::ShowResults(bool bShow)
{
    ...
    if (bShow && !isVisible)
    {
        MoveWindow(*this, windowRect.left, windowRect.top, windowRect.right - windowRect.left, height + 248, TRUE);
        POINT pt(windowRect.left, windowRect.bottom);
        ScreenToClient(*this, &pt);
        MoveWindow(hListControl, pt.x + 10, pt.y + 30, windowRect.right - windowRect.left - 20, 220, TRUE);
...
void CFindReplaceDlg::LoadSearchStrings()
{
    ...
    m_maxSearchStrings = LoadData(searchStrings, DEFAULT_MAX_SEARCH_STRINGS,
                                  SEARCHREPLACE_SECTION, L"MaxSearch", L"search%d");
...
void CFindReplaceDlg::SaveSearchStrings() const
{
    ...
    SaveData(searchStrings, SEARCHREPLACE_SECTION, L"MaxSearch", L"search%d");
}
...
void CFindReplaceDlg::LoadReplaceStrings()
{
    ...
    m_maxReplaceStrings = LoadData(replaceStrings, DEFAULT_MAX_REPLACE_STRINGS,
                                   SEARCHREPLACE_SECTION, L"MaxReplace", L"replace%d");
...
void CFindReplaceDlg::SaveReplaceStrings() const
{
    ...
    SaveData(replaceStrings, SEARCHREPLACE_SECTION, L"MaxReplace", L"replace%d");
}
...
void CFindReplaceDlg::LoadSearchFolderStrings()
{
    ...
    m_maxSearchFolderStrings = LoadData(searchFolderStrings, DEFAULT_MAX_SEARCHFOLDER_STRINGS,
                                        SEARCHREPLACE_SECTION, L"MaxSearchFolders", L"searchfolder%d");
...
void CFindReplaceDlg::SaveSearchFolderStrings() const
{
    ...
    SaveData(searchFolderStrings, SEARCHREPLACE_SECTION, L"MaxSearchFolders", L"searchfolder%d");
}
...
void CFindReplaceDlg::LoadSearchFileStrings()
{
    ...
    m_maxSearchFileStrings = LoadData(searchFileStrings, DEFAULT_MAX_SEARCHFILE_STRINGS,
                                      SEARCHREPLACE_SECTION, L"MaxSearchFiles", L"searchfile%d");
...
void CFindReplaceDlg::SaveSearchFileStrings() const
{
    ...
    SaveData(searchFileStrings, SEARCHREPLACE_SECTION, L"MaxSearchFiles", L"searchfile%d");
}
...
void findReplaceResize()
{
    if (g_pFindReplaceDlg != nullptr && IsWindowVisible(*g_pFindReplaceDlg))
        g_pFindReplaceDlg->OnSize();
}

void findReplaceClose()
{
    if (g_pFindReplaceDlg != nullptr && IsWindowVisible(*g_pFindReplaceDlg))
        EndDialog(*g_pFindReplaceDlg, 0);
}
