...
void CAutoComplete::Init()
{
    ...
    auto iniPath = CAppUtils::GetDataPath() + L"\\n4d.snippets";
    iniFiles.push_back(std::make_unique<CSimpleIni>());
    iniFiles.back()->SetUnicode();
                ...
                if (!sText.empty())
                {
                    ...
                    if (colonPos != std::string::npos)
                    {
                        ...
                        if (!sSnippet.empty())
                        {
                            ...
                            auto autoBrace = GetInt64(DEFAULTS_SECTION, L"AutoBrace");
                            SetInt64(DEFAULTS_SECTION, L"AutoBrace", 0);
                            OnOutOfScope(SetInt64(DEFAULTS_SECTION, L"AutoBrace", autoBrace));
...
void CAutoComplete::HandleAutoComplete(const SCNotification* scn)
{
    ...
    bool                  forceShow     = (scn->nmhdr.idFrom == cmdShowAutoComplete);
    ...
    else if (forceShow || (GetInt64(DEFAULTS_SECTION, L"AutoComplete") && word.size() > 1))
    {
...
LRESULT CAutoCompleteConfigDlg::DlgFunc(HWND /*hwndDlg*/, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_INITDIALOG:
        {
            ...
            auto id = m_main->GetCurrentTabId();
            if (m_main->m_docManager.HasDocumentID(id))
            {
                const auto& doc = m_main->m_docManager.GetDocumentFromID(m_main->GetCurrentTabId());
...
{
    const bool autoCompleteIgnoreCase = GetInt64(DEFAULTS_SECTION, L"AutoCompleteMatchCase", 0) != 1;
    const auto maxSearchTime          = std::chrono::milliseconds(GetInt64(DEFAULTS_SECTION, L"AutoCompleteMaxSearchTime", 2000));
